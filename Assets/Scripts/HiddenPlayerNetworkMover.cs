...
public class HiddenPlayerNetworkMover : Photon.MonoBehaviour {
	...
	void Start () {
		...
		playerShooting = GetComponentInChildren<HiddenShooting> ();
		...
		if (photonView.isMine) {
			...
			GetComponent<UnitySampleAssets.Characters.FirstPerson.HiddenFirstPersonController>().enabled = true;
			...
			for(int i = 0; i < weapons.Length; i++){
				...
				if(weapons[i].GetComponentInParent<HiddenPlayerNetworkMover>().gameObject != null){
				if(weapons[i].GetComponentInParent<HiddenPlayerNetworkMover>().gameObject.GetInstanceID() == gameObject.GetInstanceID() )
					weapons[i].layer = 10; 
				else if(weapons[i].GetComponentInParent<PlayerNetworkMover>().gameObject.GetInstanceID() == gameObject.GetInstanceID() )
						weapons[i].layer = 10; 
				}
			}
			...
			for(int i = 0; i < GameObject.FindGameObjectsWithTag("Body").Length; i++){
				...
				if(GameObject.FindGameObjectsWithTag("Body")[i].GetComponentInParent<HiddenPlayerNetworkMover>().gameObject.GetInstanceID() == gameObject.GetInstanceID() ){
			...
			for(int i = 0; i < GameObject.FindGameObjectsWithTag("Head").Length; i++){
				...
				if(GameObject.FindGameObjectsWithTag("Head")[i].GetComponentInParent<HiddenPlayerNetworkMover>().gameObject.GetInstanceID() == gameObject.GetInstanceID() ){
